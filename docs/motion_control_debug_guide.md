...
### 3.0 首次下水前置（必做，顺序固定，缺一不可）

> 下面 4 项任何一项没完成，都不要进入 mode1 闭环整定，否则调出来的参数无效甚至危险。

1. **惯导静态标定（对应安装轴与单位）**：当前代码已把 GI510 欧拉角按度转弧度（`imu_deg2rad`），
   航向安装偏置参数为 `imu_yaw_offset_deg`（默认 0）。
   首次下水前必须先做三姿态读数标定（坞内静态：艇艏朝北水平 / 右转90° / 抬头10° / 右倾10°，
   记录模块 yaw/pitch/roll），据此设定航向偏置并**确认 pitch/roll 轴映射**；
   **轴映射未确认前，roll/pitch 闭环与 DOB 的物理含义不成立，禁止进行它们的整定**。
2. **台架推进器符号验证**：逐推给正指令→确认实际推力方向与 alloc_matrix 对应列一致；
   反向则翻转该列全部符号（改参数即可）。这一步排在任何闭环之前（§3.1 模式3 检查里也要做）。
3. **DVL 锁底确认**：`connection_status=1` 只代表硬件在线；必须确认 DVL 实际锁底、
   静止时 vx/vy/vz 读数稳定（失锁时 hal 会强制归零但仍报在线，速度环会被"假零速"误导，见 §4 表）。
4. **按物理上限预置 `max_out`（防止在饱和区里整定）**：默认 max_out 大多超过物理可达上限，
   直接整定会长期饱和、参数不可信。下水前先把各轴输出限幅设为该轴物理上限的保守值：

   | 通道 | 物理来源 | 粗略上限 | 建议 max_out 初值 |
   |---|---|---|---|
   | surge (Fx) | 主推 1 路 ≤441 N | 441 N | 300 |
   | sway (Fy) | 侧推 2 路 69×2 | ≈138 N（与 yaw 共用） | 100 |
   | depth (Fz) | 垂推 **3 路** ID3+4+5 69×3 | ≈207 N（与 pitch/roll 共用） | 150 |
   | yaw (Mz) | 侧推 x 力臂 ≈1.78/1.75 m | ≈243 N·m | 170 |
   | pitch (My) | 垂推 x 力臂 1.66/1.62 m | ≈340 N·m（三推组合） | 240 |
   | roll (Mx) | 垂推 y 间距 0.15 m | ≈10 N·m | 8 |

   注：表中"上限"为同轴全力、其它轴为零时的理想值；实际多轴同时需求时还要互相让，
   所以先按建议初值跑，若从不饱和再酌情放宽；**整定期间出现长期 ±100% = 需求超限，不是加 ki 能解决的**。
...
**判读要点：**
- 某通道输出**长期顶在 ±100%** → 该轴需求超过物理推力：降 kp/ki、降低任务量或降低任务速度/阶跃（先核对 §3.0 第 4 条的 max_out 预置），并检查 `thrust_limits` 是否偏小；
...
3. depth 误差大时垂推容易饱和（垂推共 **3 路** ID3+4+5，合计 ≈207 N，且与 pitch/roll 共用），**深度阶跃别给太大**（建议 ≤1 m 起步）；
...
| 速度环异常（推力顶满但艇不动/乱动），但无任何停机日志 | **DVL 失锁被强制归零但仍报在线**（hal 侧目前行为），surge/sway 以假零速闭环 | 确认 DVL 锁底；推动 hal 在失锁时置 `connection_status=0` |
